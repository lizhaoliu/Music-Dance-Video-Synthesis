...
def draw_pose_figure(coors, height=360, width=640, limb_thickness=4):
    ...
    for joint_relation in joint_to_limb_heatmap_relationship:
        ...
        polygon = cv2.ellipse2Poly(
            coords_center, (int(limb_length / 2), limb_thickness), int(angle),
            0, 360, 1)
...
def save_batch_images(coors, batch_num,
                      save_dir_start='/mnt/external4/output_demo'):
...
def save_2_batch_images(real_coors, fake_coors, batch_num,
                        save_dir_start='/mnt/external4/output_demo'):
...
def save_2_batch_images_long(real_coors, fake_coors, batch_num,
                             save_dir_start='/mnt/external4/output_demo'):
    ...
    for row in range(rows):
        ...
        for col in range(cols):
            if (col == 0):
                ...
                col_img = cv2.resize(col_img, (0, 0), fx=0.5, fy=0.5,
