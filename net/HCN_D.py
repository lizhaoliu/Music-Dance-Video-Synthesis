...
class seq_discriminator(nn.Module):
    def __init__(self, batch):
        ...
        self.pose_rnn = nn.GRU(bidirectional=True, hidden_size=256,
                               input_size=256, num_layers=2, batch_first=True)
    ...
    def forward(self, image, audio):
        ...
        pose_out = self.pose_encoder(pose).contiguous().view(self.batch, 1, 256)
        # batch,1,256
        # print("pose_out",pose_out.shape)

        tran_audio = audio.contiguous().view(-1, 1, 1600)
        audio_out = self.audio_encoder(tran_audio)  # 1, 50, 256
        audio_out = audio_out.view(50, self.batch, -1).transpose(0, 1)  # batch,
        # 50,256
