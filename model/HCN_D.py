...
class SelfAttentiveEncoder(nn.Module):
    ...
    def forward(self, outp):
        ...
        compressed_embeddings = outp.contiguous().view(-1, size[
            2])  # [bsz*len, nhid*2]

        hbar = self.tanh(self.ws1(
            self.drop(compressed_embeddings)))  # [bsz*len, attention-unit]
        alphas = self.ws2(hbar).contiguous().view(size[0], size[1],
                                                  -1)  # [bsz, len, hop]
        alphas = torch.transpose(alphas, 1, 2).contiguous()  # [bsz, hop, len]
        alphas = self.softmax(alphas.view(-1, size[1]))  # [bsz*hop, len]
        alphas = alphas.contiguous().view(size[0], self.attention_hops,
                                          size[1])  # [bsz, hop, len]
...
class seq_discriminator(nn.Module):
    ...
    def forward(self, image, audio):
        ...
        pose = image.contiguous().view(self.batch, 50, 18, 2, 1).permute(0, 3,
                                                                         1, 2,
                                                                         4)
        # (batch,2,50,18,1) N, C, T, V, M
        pose_out = self.pose_encoder(pose).contiguous().view(self.batch, 1,
                                                             256)  # batch,1,256
        tran_audio = audio.contiguous().view(-1, 1, 1600)
        audio_out = self.audio_encoder(tran_audio)  # 1, 50, 256
        audio_out = audio_out.contiguous().view(50, self.batch, -1).transpose(0,
                                                                              1)  # batch,50,256
