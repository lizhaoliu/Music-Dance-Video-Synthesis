...
class Self_Attn(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        self.key_conv = nn.Conv1d(in_channels=in_dim, out_channels=in_dim // 8,
                                  kernel_size=1)
    ...
    def forward(self, x):
        ...
        # width*height).permute(0,2,1) # B X CX(N)
        proj_query = self.query_conv(x).view(m_batchsize, -1, N).permute(0, 2,
                                                                         1)
...
class Encoder(nn.Module):
    def __init__(self, code_size=256, rate=16000, feat_length=0.1,
                 init_kernel=0.005, init_stride=0.001, num_feature_maps=16,
        ...
        if init_kernel is not None and init_stride is not None:
            ...
            padding = calculate_padding(init_kernel * rate,
                                        ...
                                        in_size=features)
            init_features = calculate_output_size(features, init_kernel * rate,
                                                  stride=init_stride * rate,
                                                  padding=padding)
        ...
        for i in range(len(self.strides) - 1):
            padding = calculate_padding(self.kernels[i], stride=self.strides[i],
                                        in_size=features)
            features = calculate_output_size(features, self.kernels[i],
            ...
            if i == 0:
                self.cl.append(
                    nn.Conv1d(1, num_feature_maps, self.kernels[i],
                ...
                self.cl.append(nn.Conv1d(num_feature_maps, 2 * num_feature_maps,
                                         self.kernels[i],
                                         stride=self.strides[i], padding=pad))
                self.activations.append(
...
class RNN(nn.Module):
    def __init__(self, batch):
        ...
        self.rnn = nn.GRU(bidirectional=True, hidden_size=256, input_size=256,
                          num_layers=2, batch_first=True)

        # self.rnn = nn.GRU(hidden_size=256, input_size=256,num_layers= 2,
        # batch_first=True)
...
class hr_pose_generator(nn.Module):
    def __init__(self, batch, hidden_channel_num=64, input_c=266,
                 linear_hidden=1024):
